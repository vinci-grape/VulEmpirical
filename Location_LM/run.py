...
from transformers import (get_linear_schedule_with_warmup, RobertaModel, RobertaConfig, RobertaTokenizer, T5Config, T5ForConditionalGeneration, PLBartConfig, PLBartForConditionalGeneration, PLBartTokenizer)
...
def convert_examples_to_features(js, tokenizer, args):
    ...
    if args.model_type == "plbart" or args.model_type == "bert":
        code = js['func_before'].replace("\n", " NEW_LINE ") + " NEW_LINE "
    ...
    return InputFeatures(source_ids, js['location_label'], js['index'])
...
def evaluate(args, eval_dataloader, model):
    ...
    for batch in tqdm(eval_dataloader, total=len(eval_dataloader)):
        ...
        with torch.no_grad():
            lm_loss, prob, label = model(code, label)
...
def main():
    ...
    if args.model_type == "plbart":
        ...
        args.nl_ids = tokenizer('\n', return_tensors='pt', add_special_tokens=False).input_ids[0][0]
