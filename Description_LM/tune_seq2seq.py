...
def load_tokenize_data(args):
    ...
    if os.path.exists(args.cache_data):
        ...
        with open("../Datasets/train.jsonl", 'r') as file:
        ...
        def preprocess_function(examples):
            source = [' '.join(ex.strip().split()) for ex in examples["func_before"]]
            target = [' '.join(ex.strip().split()) for ex in examples["description_label"]]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max-target-len', default=200, type=int)
    parser.add_argument('--cache-data', default='cache_data/Big-Vul', type=str)
    ...
    # Logging and stuff
    parser.add_argument('--save-dir', default="saved_models/CodeT5p", type=str)
