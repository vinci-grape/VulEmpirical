...
from transformers import (get_linear_schedule_with_warmup, AutoTokenizer, AutoModelForSequenceClassification, PhiForSequenceClassification)
...
def convert_examples_to_features(js, tokenizer, args):
    ...
    return InputFeatures(input_ids, attention_mask, number_to_rating(js['assessment_label']), js['index'])
...
def evaluate(args, eval_dataloader, model):
    ...
    for batch in tqdm(eval_dataloader, total=len(eval_dataloader)):
        ...
        with torch.no_grad():
            logit = model(input_ids=input_ids, attention_mask=attention_mask)[0]
            prob = torch.nn.functional.softmax(logit, dim=-1)
            lm_loss = F.cross_entropy(logit, label)
...
def main():
    ...
    if "models--microsoft--phi-2" in args.model_name_or_path:
        model = PhiForSequenceClassification.from_pretrained(args.model_name_or_path, trust_remote_code=True, output_hidden_states=True, torch_dtype=torch.bfloat16, device_map='auto', num_labels=3)
    else:
        model = AutoModelForSequenceClassification.from_pretrained(args.model_name_or_path, trust_remote_code=True, output_hidden_states=True, torch_dtype=torch.bfloat16, device_map='auto', num_labels=3)
