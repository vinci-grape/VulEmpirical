...
def convert_examples_to_features(js, tokenizer, args):
    ...
    return InputFeatures(input_ids, attention_mask, js['label'], js['index'])
...
def evaluate(args, eval_dataloader, model):
    ...
    df.to_csv(f'../Results/Detection/{args.output_dir.split("/")[-1]}.csv', index=False)
...
def main():
    ...
    if "models--microsoft--phi-2" in args.model_name_or_path:
        model = PhiForSequenceClassification.from_pretrained(args.model_name_or_path, trust_remote_code=True, output_hidden_states=True, torch_dtype=torch.bfloat16, device_map='auto')
    else:
        model = AutoModelForSequenceClassification.from_pretrained(args.model_name_or_path, trust_remote_code=True, output_hidden_states=True, torch_dtype=torch.bfloat16, device_map='auto')
